...
发送图片时，Bot 会将图片保存到临时文件，并将路径传给 Droid 处理。Droid 会根据 AGENTS.md 中的规则决定：

- **图片分析**：使用 `mmx vision --file <路径>` 分析图片内容
- **图片上传**：使用 `curl` 上传图片到电商网站或其他存储

```
用户: [发送图片] + "这是什么？"
Bot: [调用 mmx vision 分析图片，返回结果]

用户: [发送图片] + "上传到产品 Meme Plush 01"
Bot: [调用 curl 上传图片到电商存储，返回结果]
```

**注意：** 图片处理不需要模型支持图片输入。Droid 会根据需求选择合适的工具（mmx vision 或 curl）。
